...
from .models import InfrastructureProject, NonInfrastructureProject, NonInfrastructureCategory, UserRole
...
class OfficePublicationIntegrationTests(TestCase):
    def test_both_offices_publish_and_replace_only_approved_snapshots(self):
        ...
        for office, model, prefix, title_key, payload, public_route in [
            ('engineer', InfrastructureProject, 'engineering_projects:project', 'title', fixture.valid_data(), 'public_infrastructure_project_detail'),
            ('mayor', NonInfrastructureProject, 'mayor_projects:non_infrastructure_project', 'title', {
                'title': 'Community event', 'description': 'Integration program',
                'category': category.pk, 'status': 'planned', 'proponent': 'Mayor Office',
            ...
            UserRole.objects.create(user=staff, department=office, role='staff')
            UserRole.objects.create(user=head, department=office, role='head')
            ...
            for version in [1, 2]:
                ...
                revision = record.project.revisions.order_by('-revision_number').first()
                ...
                if previous:
                    ...
                    self.assertFalse(previous.is_current_public)
                previous = revision
